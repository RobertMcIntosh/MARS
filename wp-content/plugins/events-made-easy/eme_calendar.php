...
function eme_get_calendar($args="") {
  ...
  // we'll look for events in the requested month and 7 days before and after
   ...
   $output=$calendar;
   if ($echo)
      echo do_shortcode('[expand title="Expand" swaptitle="Close" style="display: block; nospan="true"]'.$output.'[/expand]');
   else
      return do_shortcode('[expand title="Expand" swaptitle="Close" style="display: block; nospan="true"]'.$output.'[/expand]');
}
