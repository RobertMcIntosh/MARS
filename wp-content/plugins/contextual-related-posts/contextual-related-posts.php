...
function ald_crp( $args ) {
	...
	$expand = '[expand title="Expand" swaptitle="Close" style="display: block; nospan="true"]';
	$expandClose ='<span class="highlight"></span>[/expand]';
	...
	if ( $results ) {
		...
		if ( ! $is_widget ) $output .= apply_filters( 'crp_heading_title', str_replace( "%postname%", $post->post_title, $title ).$expand );
...
function crp_default_options() {
	global $crp_url;
	$title = __( '<h3 id="here">Related Posts:</h3>', CRP_LOCAL_NAME );
