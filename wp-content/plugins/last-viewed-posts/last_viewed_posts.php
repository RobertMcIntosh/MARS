...
$zg_number_of_posts = 5; // How many posts should be displayed in the list? Default is 10.
...
function zg_lw_setcookie() { // Do the stuff and set cookie
	...
	if (! isset($_COOKIE["WP-LastViewedPosts"])) {
		...
		if (! is_array($zg_cookiearray)) {
			$zg_cookiearray = array($zg_post_ID); // If array is fed up...just build a new one.
		}
	}
...
function zg_recently_viewed($output) { // Output

	$output = '<ul class="viewed_posts">';
	if (isset($_COOKIE["WP-LastViewedPosts"])) {
		...
		$zg_post_IDs = unserialize(preg_replace('!s:(\d+):"(.*?)";!e', "'s:'.strlen('$2').':\"$2\";'", stripslashes($_COOKIE["WP-LastViewedPosts"]))); // Read serialized array from cookie and unserialize it
		foreach ($zg_post_IDs as $value) { // Do output as long there are posts
			global $wpdb;
			$zg_get_title = $wpdb->get_results("SELECT * FROM $wpdb->posts WHERE post_type = 'pet' AND ID = '$value+0' LIMIT 1" );		
			foreach($zg_get_title as $zg_title_out) {
				$output .= "<li class=\"".pet_thumb."\"><a href=\"". get_permalink($value+0) ."\" title=\"". $zg_title_out->post_title . "\">". $zg_title_out->post_title."". get_the_post_thumbnail($zg_title_out->ID, 'thumbnail')."</a></li>\n"; // Output link and title
				$status=wp_get_object_terms($zg_title_out->ID, 'pet-status') ;
				$output .= "<li class=\"".pet_btn."\">
								<a href=\"". get_permalink($value+0) ."\" title=\"".'Revisit '. $zg_title_out->post_title . "\">".$status[0]->slug."
									<span class=\"".icon."".' '. $status[0]->slug."\"></span>
								</a>
							</li>\n";
			}
		}
...
function zg_lwp_widget($args) { // Widget output	
	...
	if (isset($_COOKIE["WP-LastViewedPosts"])) {
		...
		echo do_shortcode('[expand id="viewClose" title="Expand" swaptitle="Close" swapalt="Close" style="display: block;"]'
		.zg_recently_viewed($output).'<span class="collapseomatic colomat-close scroll-to-trigger" id="bot-viewClose">click here to close</span>[/expand]');
		echo $after_widget;
	}
}
...
function zg_lwp_widget_control() { // Widget control
	...
	?>
<p>
	<label for="lwp-title"> <?php _e('Title:') ?> <input type="text"
		style="width: 250px" id="lwp-title" name="lwp-title"
		value="<?php echo $title ?>" />
	</label>
</p>
<input
	type="hidden" name="lwp-submit" id="lwp-submit" value="1" />
<?php
}
