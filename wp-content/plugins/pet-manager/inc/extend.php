...
function pet_search_form() {
	...
	$searchform .= '<form action="' . get_home_url() . '/" method="get" id="searchpetform"><fieldset><ol>' .
			...
			$pet_sizes .
			'</select></li>' .
			'<li id="item-age"><label for="pet_age">' . __('Age', 'wp_pet') . '</label><select id="pet_age" name="pet-age">' .
			'<option value="0">Any</option>' .
			$pet_ages .
			'</select></li>' .
			'<input type="hidden" name="post_type" value="pet" />' .
			'<br /><input type="submit" id="searchpet" name="search" value="' . __('Search', 'wp_pet') . '">' .
			'' .
			'</ol></fieldset></form>';

	return do_shortcode('[expand title="Expand" swaptitle="Close" style="display: block; nospan="true"]'.$searchform.'[/expand]');
}
