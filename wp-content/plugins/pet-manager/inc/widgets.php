...
<?php if ( $r ) {  ?>
<span><a href="<?php echo home_url(); ?>/?feed=rss2&amp;post_type=pet"><span
		class="pet_rss">RSS</span> </a> </span>
<?php } ?>

<?php  while ($q->have_posts()) : $q->the_post(); ?>

<ul class="widget pet_info">
	<li><span class="pet_image"> <a href="<?php the_permalink() ?>"
			title="<?php the_title(); ?>"><?php the_post_thumbnail('pet_img'); ?>
		</a>
	</span>
	</li>
	<li class="pet_name"><a href="<?php the_permalink() ?>" rel="bookmark"
		title="<?php the_title(); ?>"><?php the_title(); ?> </a>
	</li>
	<li class="pet_category"><?php $category = wp_get_object_terms(get_the_ID(), 'pet-category') ; echo '<span>'.__('In', 'wp_pet') . '</span> '. $category[0]->name ; ?>
	</li>
	<li class="pet_gender"><?php $gender = wp_get_object_terms(get_the_ID(), 'pet-gender') ; echo '<span>' . __('Gender', 'wp_pet') . '</span> '. $gender[0]->name ; ?>
	</li>
	<li class="pet_age"><?php $age = wp_get_object_terms(get_the_ID(), 'pet-age') ; echo '<span>' . __('Age', 'wp_pet') . '</span> '. $age[0]->name ; ?>
	</li>
	<li class="pet_size"><?php $size = wp_get_object_terms(get_the_ID(), 'pet-size') ; echo '<span>' . __('Size', 'wp_pet') . '</span> '. $size[0]->name ; ?>
	</li>

	<li class="pet_btn">
	<a href="<?php the_permalink() ?>" title="<?php _e('Read about', 'wp_pet'); ?> <?php the_title(); ?>"> 
		<span class="icon 
			<?php $status = wp_get_object_terms(get_the_ID(), 'pet-status') ; echo $status[0]->slug ; ?>">
		</span> 
		<?php  $pd=get_the_ID(); $taxo = wp_get_object_terms($pd, 'pet-status') ; echo $taxo[0]->name ; ?>
	</a>
	</li>
</ul>
...
<?php
// Reset the global $the_post as this query will have stamped on it
wp_reset_postdata();
...
<p>
	<label for="<?php echo $this->get_field_id('text'); ?>"><?php _e('Arbitrary text or HTML'); ?>
	</label>
...
<p>
	<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('How many items would you like to display?'); ?>
	</label> <select id="<?php echo $this->get_field_id('number'); ?>"
...
<p>
	<label for="<?php echo $this->get_field_id('sortby'); ?>"><?php _e( 'Sort by:' ); ?>
		...
		<option value="title"
		<?php selected( $instance['sortby'], 'title' ); ?>>
			<?php _e('Post title','wp_pet'); ?>
		</option>
		<option value="date" <?php selected( $instance['sortby'], 'date' ); ?>>
			<?php _e('Post data','wp_pet'); ?>
		</option>
		<option value="author"
		<?php selected( $instance['sortby'], 'author' ); ?>>
			<?php _e( 'Post author','wp_pet'); ?>
		</option>
		<option value="ID" <?php selected( $instance['sortby'], 'ID' ); ?>>
			<?php _e( 'Post ID','wp_pet'); ?>
		</option>
		<option value="rand" <?php selected( $instance['sortby'], 'rand' ); ?>>
			<?php _e( 'Random' ); ?>
		</option>
		<option value="modified"
		<?php selected( $instance['sortby'], 'modified' ); ?>>
			<?php _e( 'Modified date','wp_pet'); ?>
		</option>
		<option value="comment_count"
		<?php selected( $instance['sortby'], 'comment_count' ); ?>>
			<?php _e( 'Popularity','wp_pet'); ?>
		</option>
	</select>
</p>
...
<p>
	...
	<select class="widefat"
		id="<?php echo $this->get_field_id('pet-category'); ?>"
		name="<?php echo $this->get_field_name('pet-category'); ?>">
		<option value="">
			<?php _e('All','wp_pet'); ?>
		</option>
		<?php
...
<p>
	...
	<select class="widefat"
		id="<?php echo $this->get_field_id('pet-status'); ?>"
		name="<?php echo $this->get_field_name('pet-status'); ?>">
		<option value="">
			<?php _e('All','wp_pet'); ?>
		</option>
		<?php
...
<input type="checkbox" class="checkbox"
	...
	<?php checked( $rss ); ?> />
<label for="<?php echo $this->get_field_id('rss'); ?>"><?php _e( 'Show RSS link','wp_pet' ); ?>
</label>
<br />
<?php
    }
